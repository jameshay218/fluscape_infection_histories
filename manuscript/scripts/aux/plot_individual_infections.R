...
p_infection <- ggplot(wow) + 
  ...
  ylab("Individual, ordered by age") +
  xlab("")
p_infection
...
ar_estimates_old <- ar_estimates[ar_estimates$time < min(titre_dat$samples),]
...
total_infs_old <- total_infs[total_infs$time < min(titre_dat$samples),]



data.table::setkey(total_infs_recent, "sampno")
data.table::setkey(total_infs_old, "sampno")
...
ar_estimates_overall_old <- total_infs_old[,list(lower_quantile=quantile(ar,c(0.025)),
                                                       median=quantile(ar,c(0.5)),
                                                       upper_quantile=quantile(ar,c(0.975)),
                                                       precision=1/var(ar),
                                                       var=sd(ar)/mean(ar)
),by=key(total_infs_old)]

quantile(ar_estimates_overall_recent$median*100, c(0.5,0.025,0.975))
quantile(ar_estimates_overall$median*100, c(0.5,0.025,0.975))
quantile(ar_estimates_old$median*100, c(0.5,0.025,0.975))
