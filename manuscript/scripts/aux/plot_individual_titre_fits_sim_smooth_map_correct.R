...
sim_inf_hist <- read_csv("~/Documents/GitHub/fluscape_serosolver/data/simulated/sim_quarterly_fluscape_supplement_2_infection_histories.csv")

#real_par_tab <- read_csv(paste0(main_wd,"../data/simulated/sim_quarterly_fluscape_clusters_par_tab.csv"))
real_par_tab <- read_csv("~/Documents/GitHub/fluscape_serosolver/data/simulated/sim_quarterly_fluscape_supplement_2_par_tab.csv")
...
sim_true_ar <- read_csv("~/Documents/GitHub/fluscape_serosolver/data/simulated/sim_quarterly_fluscape_supplement_2_attack_rates.csv")
...
ggsave_jah(p_titre_fit_comb,figure_wd,"sim_titre_fits_example_smooth_map_correct",width=7,height=8)

## Find RMSE takes a very long time
if(FALSE){
...
p_sim_ar <- plot_attack_rates_monthly(inf_chain %>% mutate(group=1) %>% filter(sampno > 1000),sim_data %>% mutate(group=1),
...
antigenic_map_clustered <- read.csv("~/Documents/GitHub/fluscape_serosolver/data/antigenic_maps/antigenic_map_fonville_quarterly_clustered_clustered.csv")
antigenic_map_clustered <- antigenic_map_clustered %>% mutate(cluster_change = x_coord != lag(x_coord,1))


p_sim_ar <- p_sim_ar + geom_line(data = sim_true_ar, aes(x = year, y = AR),col = "black", size = 0.25) + ylab("Per capita incidence per quarter") +
  geom_vline(data=antigenic_map_clustered %>% filter(cluster_change == TRUE),aes(xintercept=inf_times),linetype="dashed",col="purple") +
...
colnames(inf_chain) <- c("individual","time","x","sampno","chain_no")
## Convert time incidences to annual
inf_chain <- inf_chain %>% mutate(time = floor((time-1)*(1/4)) + 1)
setkey(inf_chain, "sampno","individual","time","chain_no")
inf_chain <- inf_chain[,list(x=as.numeric(any(x==1))),by=key(inf_chain)]
...
compare_ar_annual <- inf_chain %>% group_by(time,sampno, chain_no) %>% 
...
compare_ar_annual_smooth_map_correct <- compare_ar_annual %>% mutate(Model="Smooth map correct")
...
ggsave_jah(p_sim_recovery_alt,figure_wd,"p_sim_recovery_all_smooth_map_correct",8,8)
ggsave_jah(p_sim_ar_annual,figure_wd,"p_sim_recovery_all_smooth_map_correct_annual",7,4)
